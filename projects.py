...
for p in range(1, 21):
    ...
    if os.path.exists(pxml_file):
        ...
        url = "https://www.openhub.net/projects.xml?api_key="\
            "58c554bf40d74f151057d1b276b22f0c5ccadc77b79c1fd762cdbe5fadfeb26a"\
            "&sort=rating&page={}".format(p)
    ...
    if fetched_data_from_url:
        ...
        records = [{x.tag: x.text.strip() for x in proj if x.text}
...
for p in range(1, 21):
    ...
    if os.path.exists(msgfile_path):
        ...
        print("{}.msg is missing, check the script and rerun script".format(
            msgfile_path))
        success = False
        break
